...
依赖: pip install mcp
"""
import asyncio, subprocess, os, http.server, socketserver, threading, json
...
async def _run(args):
    ...
    try:
        result = subprocess.run(cmd + extra_args, capture_output=True, text=True, timeout=timeout, cwd=str(path.parent))
        out = []
        if result.stdout:
            out.append(f"📤 STDOUT:\n{result.stdout[:4000]}")
        if result.stderr:
